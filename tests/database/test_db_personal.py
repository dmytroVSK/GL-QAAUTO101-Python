...
def test_qnt_orders(db):
    ...
    db.clean_orders_tbl()
    db.cursor.executemany("INSERT INTO orders (customer_id, product_id) VALUES (?, ?)", orders)
...
def test_popular_product_in_city(db):
    ...
    assert result == ('Kyiv', 'солодка вода', 'з цукром', 2)
...
def test_not_sold_products(db):
    ...
    assert (row[0][0], row[1][0], row[2][0]) == (3,4,5)
